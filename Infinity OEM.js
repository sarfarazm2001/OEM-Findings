(function () {
  const parts = [
    ...
    { name: "Canon Motor", pn: "YB06-00300", customName: "Canon Motor YB06-00300" },
  ...
  parts.forEach((p, i) => {
    html += `<label style="display:flex;align-items:center;justify-content:space-between;padding:4px 0;font-size:12px;cursor:pointer;"><span style="display:flex;align-items:center;"><input type="checkbox" class="inf_part" data-eval="${
      p.isEval ? "1" : "0"
    }" value="${p.customName || p.name}" style="margin-right:8px;transform:scale(1.1);">${
      p.name
    }</span><span style="color:#666;font-family:monospace;">${p.pn}</span></label>`;
  });

  html += `</div><div style="display:flex;gap:8px;"><button id="inf_copy" style="flex:1;padding:10px;background:#28a745;color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:13px;font-weight:bold;">Apply Findings</button><button id="inf_cancel" style="flex:1;padding:10px;background:#dc3545;color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:13px;font-weight:bold;">Cancel</button></div>`;

  box.innerHTML = html;
  overlay.appendChild(box);
  document.body.appendChild(overlay);

  document.getElementById("inf_cancel").onclick = () =>
    document.body.removeChild(overlay);

  document.getElementById("inf_copy").onclick = () => {
    let selected = Array.from(box.querySelectorAll(".inf_part:checked"))
      .filter((cb) => cb.getAttribute("data-eval") !== "1")
      .map((cb) => cb.value);
