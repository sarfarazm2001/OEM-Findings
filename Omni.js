(function () {
  ...
  box.style.cssText =
    "background:#fff;padding:20px;border-radius:8px;box-shadow:0 4px 10px rgba(0,0,0,0.3);width:550px;text-align:left;color:#333;";
  ...
  html += `</div><div style="margin-bottom:12px;text-align:center;"><button id="o_oem_only" style="width:100%;padding:8px;background:#6c757d;color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:12px;font-weight:bold;">Set to "Returned from OEM after required repairs"</button></div><div style="display:flex;gap:8px;"><button id="o_copy" style="flex:1;padding:10px;background:#28a745;color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:13px;font-weight:bold;">Apply Findings</button><button id="o_cancel" style="flex:1;padding:10px;background:#dc3545;color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:13px;font-weight:bold;">Cancel</button></div>`;
  ...
  const attachRowEvents = (row) => {
    row.querySelector(".o_edit_btn").onclick = () => {
      let newDesc = prompt(
        ...
        row.querySelector(".desc_txt").innerText
      );
      if (newDesc !== null && newDesc.trim() !== "") {
        let newPn = prompt(
          "Edit Part Number:",
          row.querySelector(".pn_txt").innerText
        );
    ...
    row.querySelector(".o_del_btn").onclick = () => {
      row.remove();
    };
  };
  ...
  document.getElementById("o_add_btn").onclick = () => {
    ...
    if (!desc || !pn) {
      alert("Both Part Description and Part # are required.");
      return;
    }
    ...
    row.style.cssText =
      "display:flex;align-items:center;justify-content:space-between;padding:4px 0;font-size:12px;border-bottom:1px solid #f0f0f0;background:#eef6ff;";
  ...
  document.getElementById("o_oem_only").onclick = () =>
    applyText("Returned from OEM after required repairs.");

  document.getElementById("o_cancel").onclick = () =>
    document.body.removeChild(overlay);

  document.getElementById("o_copy").onclick = () => {
    let selectedParts = Array.from(
      box.querySelectorAll(".o_chk:checked")
    ).map((cb) => cb.value);
    let text = selectedParts.length
      ? `Per the OEM, the following parts have been replaced: ${selectedParts.join(
          ", "
        )} and passed the related testing.`
      : "Returned from OEM after required repairs.";
    applyText(text);
  };
})();
